...
pygame.init()
window = pygame.display.set_mode((WIDTH, HEIGHT)) # Creates the window outside the init so that it can be accessed to convert images
...
class Mayhem:
    ...
    def reset(self):
        print(self.rocket)
        self.rocket.sprite.reset(R0_START_POS)
        self.rocket2.sprite.reset(R1_START_POS)
    ...
    def draw_stats(self):
        ...
        window.blit(text, (WIDTH - text.get_width() - 5, 120))
        window.blit(text2, (WIDTH - text2.get_width() - 5, 150))
    ...
    def run(self):
        ...
        while run:
            ...
            self.smokegroup.draw(window)
            self.rocket.draw(window)
            self.rocket2.draw(window)
            self.p0bullets.draw(window)
            self.p1bullets.draw(window)
...
class Rocket(pygame.sprite.Sprite):
    ...
    """
    Reset the rocket's fuel, health, position, speed, and rotation.
    """
    def reset(self, pos):
        self.x, self.y = pos
        self.rotation = 0
    ...
    """
    Update the rocket's position, rotation, and fuel.
    """
    def update(self):
        # Moving
        keys = pygame.key.get_pressed()
